...
class OpenAIStreamlitApp:
    def __init__(self):
        ...
        # Define the assistant with a tool to get coordinates from an image
        self.assistant = self.client.beta.assistants.create(
            instructions="You are an assistant that can analyze images and provide coordinates for specific areas.",
            model=default_model,
            tools=[
                {
                    "type": "function",
                    "function": {
                        ...
                        "description": "Get the coordinates of a specified area in the image",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "image": {
                                ...
                                "area": {
                                    "type": "string",
                                    "description": "The area to find, e.g., 'dialog box', 'highlighted text'"
                                }
                            },
                            "required": ["image", "area"]
                        }
    ...
    def get_coordinates(self, image_path, area):
        """Uses the assistant's tool to get coordinates of a specific area in the image."""
        base64_image = self.encode_image(image_path)
        response = self.assistant.call_tool(
            "get_coordinates",
            {"image": base64_image, "area": area}
        )
        st.warning("Highlight tool used")  # This warns that the tool has been used
    ...
    def run(self):
        ...
        if uploaded_file is not None:
            ...
            area_to_highlight = st.text_input("Area to highlight (e.g., dialog box):", value="dialog box")

            if st.button('Get Coordinates and Draw Rectangle'):
                # Use the assistant tool to get coordinates based on the image and specified area
                coords = self.get_coordinates(image_path, area_to_highlight)
                ...
                st.success(f"Rectangle drawn over the {area_to_highlight} with coordinates: {used_coords}")
